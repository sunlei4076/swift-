...
class DOAlertController : UIViewController, UITextFieldDelegate, UIViewControllerTransitioningDelegate {
        ...
        if (action.style == DOAlertActionStyle.Cancel) {
            for ac in actions as! [DOAlertAction] {
                if (ac.style == DOAlertActionStyle.Cancel) {
                    let error: NSError = NSError(domain: "nil", code: 0, userInfo: nil);                    NSException.raise("NSInternalInconsistencyException", format:"DOAlertController can only have one action with a style of DOAlertActionStyleCancel", arguments:getVaList([error ?? "nil"]))
                    return
                }
            }
        }
    ...
    func addTextFieldWithConfigurationHandler(configurationHandler: ((UITextField!) -> Void)!) {
        ...
        if (!isAlert()) {
            let error: NSError = NSError(domain: "nil", code: 0, userInfo: nil)
