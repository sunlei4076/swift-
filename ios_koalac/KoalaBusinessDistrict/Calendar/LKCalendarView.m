...
-(void)turnRight
{
    ...
    [_scrollView setContentOffset:CGPointMake(scrollWidth*2, 0) animated:true];
}

+(NSInteger)monthDiffWithDate:(NSDateComponents *)date1 toDate:(NSDate *)date2
{
    return monthDiffWithDateComponents(date1, getYearMonthDateComponents(date2));
}
...
-(void)initViews
{
    ...
    _scrollView.backgroundColor = GlobalStyleFontColor;
...
{
    [self setCurrentDateComponents:getYearMonthDateComponents(date)];
}
...
-(void)setCurrentDateComponents:(NSDateComponents *)com animation:(BOOL)animation
{
    ...
    {
        long int monthdiff = monthDiffWithDateComponents(_currentDateComponents,com);
        if(monthdiff==0)
            return;
...
-(id)moveToDate:(NSDate *)date
{
    ...
    long int  index = [monthView.dates indexOfObject:date];
