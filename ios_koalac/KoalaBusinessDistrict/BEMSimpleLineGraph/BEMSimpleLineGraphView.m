...
- (void)commonInit {
    ...
    _gradientBottom = CGGradientCreateWithColorComponents(colorspace, components, locations, num_locations);
...
- (UILabel *)xAxisLabelWithText:(NSString *)text atIndex:(NSInteger)index {
    ...
    if (index == 0) {
        horizontalTranslation = lRect.size.width/2+1;
    } else if (index+1 == numberOfPoints) {
        horizontalTranslation = -(lRect.size.width/2+1);
    } else horizontalTranslation = 1;
...
- (void)displayPermanentLabelForPoint:(BEMCircle *)circleDot {
    ...
    permanentPopUpLabel.textColor = [UIColor yellowColor];
...
- (CGFloat)yPositionForDotValue:(CGFloat)dotValue {
    ...
    if (self.minValue == self.maxValue && self.autoScaleYAxis == YES){
        if(self.maxValue == 0){
            positionOnYAxis = self.frame.size.height - padding/2 - 20;
        }else{
            positionOnYAxis = (self.frame.size.height-20)/2;
        }
    }else if (self.autoScaleYAxis == YES){
        positionOnYAxis = ((self.frame.size.height - padding/2) - ((dotValue - self.minValue) / ((self.maxValue - self.minValue) / (self.frame.size.height - padding - 20))));
        positionOnYAxis -= 20;
    }
    else {
