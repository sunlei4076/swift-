...
-(void)keyboardWillShowOrHide:(NSNotification *)notification {
	...
	if (IOS_VERSION >= 3.2) {
		...
		keyboardBoundsValue = [[notification userInfo] objectForKey:UIKeyboardAnimationCurveUserInfoKey];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    if ([self.boardDelegate respondsToSelector:@selector(alttextField:shouldChangeCharactersInRange:replacementString:)]) {
        [self.boardDelegate alttextField:textField shouldChangeCharactersInRange:range replacementString:string];
    }
    return YES;
}
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if([self.boardDelegate respondsToSelector:@selector(altextView:shouldChangeTextInRange:replacementText:)]){
        return [self.boardDelegate altextView:textView shouldChangeTextInRange:range replacementText:text];
    }else{
        return YES;
    }
}
...
{
    if([self.boardDelegate respondsToSelector:@selector(altextViewDidBeginEditing:)]){
        [self.boardDelegate altextViewDidBeginEditing:textView];
    }
}
...
- (void)setBoardDelegate:(id<UIKeyboardViewControllerDelegate>)boardDelegateT
{
    _boardDelegate = boardDelegateT;
    if ([self.boardDelegate isKindOfClass:[UIViewController class]]) {
        objectView = [(UIViewController *)[self boardDelegate] view];
    }
    else if ([self.boardDelegate isKindOfClass:[UIView class]]) {
